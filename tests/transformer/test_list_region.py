"""Test for the list region endpoint."""
import filecmp
import os
...
def test_V2ListRegion(runner, tmp_path):
    """Run transformer end to end and generate a list with all available
    regions for the specific version."""
    ...
    assert filecmp.cmp(
        f"{pwd}/tests/transformer/testdata/expected/v2/os/rhel/provider/google/version/7/region/list",
        f"{tmp_path}/v2/os/rhel/provider/google/version/7/region/list",
    )
...
def test_transformerV2ListVersion(tmpdir):
    ...
    runner = transformer.transform.TransformerV2ListRegionByVersion(connection)
    chunk_size = 2
