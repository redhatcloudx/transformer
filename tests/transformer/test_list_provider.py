"""Test for the list provider endpoint."""
import filecmp
import os
...
def test_V2ListProvider(runner, tmp_path):
    """Run transformer end to end and generate a list with all available
    providers for the specific os."""
    ...
    assert filecmp.cmp(
        f"{pwd}/tests/transformer/testdata/expected/v2/os/rhel/provider/list",
        f"{tmp_path}/v2/os/rhel/provider/list",
    )


def test_transformerV2ListProvider(tmpdir):
    """Test list provider run method."""
    temporary_directory = tmpdir.mkdir("test")
    connection = transformer.connection.ConnectionFS(temporary_directory, [])
    runner = transformer.transform.TransformerV2ListProviderByOS(connection)
    chunk_size = 2
    runner.chunk_size = chunk_size
    data = [
        transformer.connection.DataEntry(
            "v2/os/rhel/provider/azure/version/8/region/global/image/dba7673010f19a94af4345453005933fd511bea9",
            {
                "date": "2019-01-01",
                "name": "test1",
                "arch": "arch1",
    ...
    expected = transformer.connection.DataEntry(
        "v2/os/rhel/provider/list",
        {"aws": 1, "azure": 1, "google": 1},
    )
